...
- **🔒 Loop Mode ON** — blinders on, cooking one dish; micro-memory = ONLY this site's cuisine pack + this dish. **The browser instance binds the site** (multi-instance model — see Run/dev): the WhatsApp instance/session does ONLY WhatsApp; cross-site means a *different* session, never a wider context. Off-dish → don't improvise; stop and hand back (fail-fast beats wandering). **Always mop** as the closing step — TWO parts: (1) **the station** — close any dialog/**modal** left open (web *or* native, e.g. a "success" popup), leave the page on a clean view; (2) **scratch** — drafts, screenshots, `.txt`/`.err` dumps (keep tools/data/logs). **Write all scratch to `runs/scratch/`** (ad-hoc shots: `loop shot scratch/<name>`). **Mopping is harness-enforced at halt, not memory-dependent:** the **Stop hook** (`.claude/settings.json`, per-machine) always wipes `runs/scratch/`, and when `LOOP_CDP_PORT` is bound (the instance.sh launch) also runs **`loop mop`** — closes web panels/modals by accessible name (never Escape), then **health-checks and prints PASS/⚠**. `loop mop` is also a first-class command, runnable anytime; scoped to `LOOP_CDP_PORT` (never touches another instance), self-times-out, wipes scratch even with the browser down. Native OS dialogs still need a manual `loop os-dismiss` (invisible to CDP). Collabs without the hook just `loop mop` by hand.
...
- `porter.mjs` (ingredient gathering) · `servicelog.mjs` (the Service Log) · `stats.mjs` (the graduation ledger — per-recipe clean-run counts; gitignored state in `dishes/`).
...
- **Two verification layers:** `npm test` (`tests/run.mjs`) is the fast offline CI harness (no browser needed; `LOOP_DISHES_DIR` sandboxes the ledger). But the real proof is **run a recipe live and watch** — `node cli.mjs <verb>` is the smoke test; a green `npm test` never substitutes for a live cook.
