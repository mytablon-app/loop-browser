...
## Flow: batch spotlight post (6-people @mention format, `cook-linkedin-batch.mjs`)
Same admin composer, different tagging mechanism: type `@`+name **directly in the caption textbox** (Quill-based `ql-mention`), not the photo Tag dialog. Confidence bar = candidate's visible headline snippet contains a 30-char slice of the person's Voyager headline; zero/multiple hits → leaves plain text (safe default, never guesses).
- ⚠ **The bottom-right blue "Post" submit button is NOT uniquely findable by name** — the composer header's audience-selector button is labeled `"Tablon Community Tablon Community Post to Anyone"`, which also matches a `/Post/i` regex and sits earlier in the DOM, so `click "Post"` grabs it and opens the **"Post settings" dialog** instead of submitting (harmless if caught — its own `Done` stays disabled until a setting changes; click **`Back`** to return to the composer unharmed). **Use `click-xy` on the actual blue Post button's pixel coords instead.**
- ⚠ **Headline-substring matching has a real false-positive mode:** confirmed live (2026-09-07) it mistagged "Vidur Bhutani" as a *different* real person, "Vidur Anand" — same first name, coincidentally-overlapping headline text, script logged "tagged ✓" with full confidence. **Always screenshot + verify the tagged display name in the draft matches the intended person before posting** — a "tagged ✓" log line is not proof of the right person.
- ⚠ **Do NOT fix a wrong mention with raw `Backspace` key presses** — confirmed live: one Backspace right after an atomic-looking mention span only trimmed the last word (not the whole node), and continuing to backspace through it cascaded into deleting the *entire adjacent paragraph* (a correctly-tagged person vanished too). **Safe fix:** click right after the mention, then in-page `Selection.modify("extend","backward","character")` for exactly `text.length` chars, verify the removed length matches before `deleteContents()`, then `keyboard.insertText(...)` the replacement — mirrors the driver's own no-match cleanup routine (`cook-linkedin-batch.mjs`), which explicitly exists because raw Backspace is unreliable here.
- If an edit goes wrong, discarding is cheap and safe: composer's top-right **X → "Save this post as a draft?" → Discard** fully resets to a clean admin page; just re-run the driver (it re-parses the pantry `.txt`, dedup is against `posted-log.txt` not in-session state).
- The driver **never auto-posts** — it always stops with the draft open for human review, per house rule (never autopost without explicit OK).
